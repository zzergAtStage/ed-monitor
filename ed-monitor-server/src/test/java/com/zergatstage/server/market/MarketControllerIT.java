...
class MarketControllerIT {
    ...
    void postGetAndUpdateMarket() {
        ...
        // POST list
        ResponseEntity<MarketDto[]> postRes = rest.postForEntity(url("/api/markets"), List.of(market), MarketDto[].class);
        assertEquals(HttpStatus.CREATED, postRes.getStatusCode());
        assertNotNull(postRes.getBody());
        assertTrue(postRes.getBody().length >= 1);

        // GET list
        ResponseEntity<MarketDto[]> getRes = rest.getForEntity(url("/api/markets"), MarketDto[].class);
        ...
        ResponseEntity<MarketDto> putRes = rest.exchange(url("/api/markets/" + toUpdate.getMarketId()), HttpMethod.PUT, req, MarketDto.class);
